...
class ZeroCouponBond(object):
    __doc__ = ' A simple zero coupon bond'
...
class BlackScholes(object):
    __doc__ = 'basic black scholes formulae'
...
class CDS(object):
    ...
    def dates(self, inc0=False):
        fd = np.arange(self.dt, self.maturity + self.dt -
                       0.0001, self.dt) - self.accruedtime
        if inc0:
...
def iterboot(bms, pfunc, x0, lbd=0.0, bds=[-1, 1], its=5, mixf=0.0, make_curve=lin.RationalTension):
    ...
    for it in range(0, its):
        ...
        tsit.build(mixf * px + (1.0 - mixf) * tsit.x,
                   mixf * py + (1.0 - mixf) * tsit.y)
        px = np.copy(tsit.x)
        py = np.copy(tsit.y)
        es.append(pve)
...
def pert_bmk(bms, ir, lbd, its=3, pert=0.0001):
    ...
    pvf = cdspv(ir, zero2disc)
    bms_ps = {CDS(k.maturity, k.coupon - pert, k.recovery)
                  : v for k, v in list(bms.items())}
