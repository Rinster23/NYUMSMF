...
def gradient_descent(pic, model, loss, iterations=256):
    learning_rate = 0.004
    wd = 1e-5
    for _ in range(iterations):
        out = model(normalize_and_jitter(transforms.GaussianBlur(9)(pic)))
        ls = -loss(out) + wd * torch.sum(pic * pic)
        ls.backward()
        grad = pic.grad
        smooth_grad = transforms.GaussianBlur(9)(grad)
        g_std = torch.std(smooth_grad)
        g_mean = torch.mean(smooth_grad)
        smooth_grad = (smooth_grad - g_mean) / g_std
        pic.data -= learning_rate * smooth_grad
        pic.grad.zero_()
        pic.data = torch.clamp(pic, 0, 1)
        print(ls)
    return pic
